...
- Physical processing release exposes immutable per-manifest OSR candidates and a typed physical command alongside the unchanged legacy order-centric debug command.
...
- Generic station processing now claims one exact station-arrival FIFO head only after downstream
  acceptance, then uses one simulation-thread-owned coordinator for active claims and FIFO immutable
  `CONTINUE`/`CONSUME` dispositions. Third Party and Adapting retain their real domain controllers;
  COLLECT preserves the exact replacement load plan, while Adapting STORE and P2P terminate the
  inbound lifecycle only after their lifecycle contracts succeed.
- P2P claims begin at tipper-input acceptance and complete only at actual tipper completion.
  Consumed inbound totes are held/hidden and remain separate from outbound allocation.
- Generic station route continuation drains one global disposition FIFO: terminal `CONSUME` is
  presented and acknowledged once; `CONTINUE` selects a pure next station and exact target, preserves
  the source-neutral release request/current plan/tote/renderable/follower, and re-enters through the
  shared common-entry transport path without duplicate publication.
...
- The operational EMPTY end-to-end proof is complete and verified on
  `feature/dsp-operational-empty-end-to-end-proof`. Its one production-boundary integration
  scenario proves AV02 allocation, shared OSR/AV02 release, real station journeys and continuation,
  exact P2P completion, and independent outbound tote allocation without production-code changes.
  The branch is pending merge to `master`; full-day execution, metrics, and inspection follow it.
  Station-to-station visual topology, renderer integration, outbound dispatch/32R, Exception
  handling, and MANUAL/MANUAL_MERGE handling remain deferred.
...
- Physical departure is an explicit simulation-thread commit performed only after downstream release acceptance.
...
19. operational EMPTY end-to-end proof (complete and verified on
    `feature/dsp-operational-empty-end-to-end-proof`, pending merge to `master`);
20. full-day analysis, metrics, and inspection as the next separately planned branch after that
    merge.
...
- operational EMPTY end-to-end proof: complete and verified on `feature/dsp-operational-empty-end-to-end-proof`, pending merge to `master`, with detailed plan at `docs/scheduler/dsp-operational-empty-end-to-end-proof-plan.md`; the locked production change surface is empty and the feature creates one scenario test class;
- full-day execution, metrics, and inspection: next separately planned programme branch after that proof is merged;
...
- operational EMPTY end-to-end proof: complete and verified on
  `feature/dsp-operational-empty-end-to-end-proof`; pending merge to `master`
- full-day execution, metrics, and inspection: next separately planned programme branch after that
  proof is merged
...
Lower-capability implementation subagents run plan-authorized focused Gradle tasks, while the higher-capability parent owns complete-step review and acceptance against the active plan. The parent normally relies on the subagent's reported focused result and does not duplicate a green run. The user runs broad regression, full-suite Gradle tasks, and visual checks and reports the results of user-run verification.
