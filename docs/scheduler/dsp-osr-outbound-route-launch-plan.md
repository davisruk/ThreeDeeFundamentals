...
smoke checks remain to be confirmed before merge.
...
- [ ] confirm legacy P2P, Adapting, Third Party, and `ALT+R` visual/reset smoke checks are green;
...
- Legacy target-specific route-entry queues remain available only through the explicit compatibility
  runtime path.
- Focused launch/transport/release/scheduler/lifecycle/OSR tests and the complete Gradle suite are
  green. Visual/reset smoke verification has not yet been reported for this branch.
